...
enum response_state
new_line(struct escape_response_parser *p, const uint8_t c) {
    if(c == '.'){
        return response_dot;
    }
    else {
...
enum response_state
dot(struct escape_response_parser *p, const uint8_t c, buffer *b) {
    p->response_size_i++;
    buffer_write(b, c);
    if(c == '\r') {
...
enum response_state
dot_cr(struct escape_response_parser *p, const uint8_t c) {
    if(c == '\n') {
...
enum response_state
byte(struct escape_response_parser *p, const uint8_t c) {
    if(c == '\r') {
        return response_cr;
    } else {
        return response_byte;
...
enum response_state
cr(struct escape_response_parser *p, const uint8_t c) {
    if(c == '\n'){
